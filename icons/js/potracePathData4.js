 ...
 * 
 *   getSVG(size, returnPathData) : return full svg markup or pathData array
 */
...
const Potrace = (function () {
    ...
    function processPath() {
        ...
        function optiCurve(path) {
            ...
            function opti_penalty(path, i, j, res, opttolerance, convc, areac) {
                ...
                let p0_ = curve.c[mod(i, m) * 3 + 2];
                let p1_ = vertex[mod(i + 1, m)];
                let p2_ = vertex[mod(j, m)];
                let p3_ = curve.c[mod(j, m) * 3 + 2];
    ...
    function getSVG(scale, returnPathData = true) {

        let len = pathlist.length;
        let w = bm.w * scale, h = bm.h * scale;
        let pathData = []
